...
export default async function AdminPage() {
  ...
  try {
    ...
    const [dashboard, aiDashboard, securityReport] = await Promise.all([
      getDashboardData(),
      getAiDashboardData(),
      getSecurityReport(),
    ]);
    return (
      <AdminDashboard
        ...
        initialAiData={aiDashboard}
        initialSecurityReport={securityReport}
        releases={APP_RELEASES}
        githubUrl={githubRepositoryUrl()}
      />
    );
  } catch {
    return <AdminLogin googleEnabled={isGoogleAuthConfigured()} />;
  }
}
